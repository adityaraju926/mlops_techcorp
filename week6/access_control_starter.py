...
class AccessController:
    ...
    def __init__(self, access_policy_path: str):
        ...
        if "document_access" not in self.policy:
            self.policy["document_access"] = {
    ...
    def can_view_document(self, role: str, document: Dict[str, Any]) -> bool:
        ...
        """
        sensitivity = document.get("sensitivity", "Restricted")
    ...
    def can_view_field(self, role: str, field_name: str) -> bool:
        ...
        if field_name not in sensitive_fields:
            return True
        visibility = sensitive_fields[field_name].get("visibility", [])
        if role in visibility:
            return True
        role_perms = self.policy.get("roles", {}).get(role, {}).get("permissions", {})
        if field_name == "salary" and role_perms.get("view_other_salaries", False):
            return True
        return False
    ...
    def redact_response(self, role: str, response: str) -> str:
        ...
        for field_name in sensitive_fields:
            if not self.can_view_field(role, field_name):
                pattern = rf'(?i)(\b{re.escape(field_name)}\b[^\n]{{0,50}}?)(\$\d[\d,]*|\d[\d,]*)'
    ...
    def log_access(self, role: str, resource: str, allowed: bool, field: str = None):
        ...
        - allowed (True/False)
        """
        self.audit_log.append({"timestamp": datetime.utcnow().isoformat(), "role": role, "resource": resource, "field": field, "allowed": allowed,})
...
class RateLimiter:
    ...
    def is_allowed(self, user_id: str) -> bool:
        ...
        self.user_query_times[user_id] = [t for t in self.user_query_times[user_id] if now - t < 60]
...
class CostEnforcer:
    ...
    def __init__(self, policy_path: str = None):
        ...
        """
        self.role_budgets = {"engineer": 100.0, "manager": 500.0, "hr": 200.0, "finance": 500.0, "executive": 1000.0,}
        self.user_spending = {}
    ...
    def can_afford_query(self, user_id: str, estimated_cost: float) -> bool:
        ...
        if user_id not in self.user_spending:
            return True
        user_data = self.user_spending[user_id]
        budget = self.role_budgets.get(user_data["role"], 0.0)
        remaining = budget - user_data["total"]
    ...
    def get_budget_remaining(self, user_id: str) -> float:
        ...
        user_data = self.user_spending[user_id]
        budget = self.role_budgets.get(user_data["role"], 0.0)
        return max(0.0, budget - user_data["total"])
